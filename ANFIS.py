...
class ANFIS:
    ...
    def __init__(self, X, Y, memFunction):
        ...
        self.rules = np.array(
            list(itertools.product(*self.memFuncsByVariable)))
        self.consequents = np.empty(
            self.Y.ndim * len(self.rules) * (self.X.shape[1] + 1))
        self.consequents.fill(0)
        self.errors = np.empty(0)
        self.memFuncsHomo = all(len(i) == len(
            self.memFuncsByVariable[0]) for i in self.memFuncsByVariable)
    ...
    def LSE(self, A, B, initialGamma=1000.):
        ...
        for i in range(len(coeffMat[:, 0])):
            a = coeffMat[i, :]
            b = np.array(rhsMat[i])
            S = S - (np.array(np.dot(np.dot(np.dot(S, np.matrix(a).transpose()),
                     np.matrix(a)), S)))/(1+(np.dot(np.dot(S, a), a)))
            x = x + (np.dot(S, np.dot(np.matrix(a).transpose(),
                     (np.matrix(b)-np.dot(np.matrix(a), x)))))
        return x

    def trainHybridJangOffLine(self, epochs, tolerance=1e-5, initialGamma=1000, k=0.033):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            print("Epoch:", epoch, 'current error --> ' + str(error))
            ...
            if convergence is not True:
                ...
                dE_dAlpha = list(backprop(self, colX, cols, wSum, w, layerFive)
    ...
    def plotResults(self):
        if self.trainingType == 'Not trained yet':
            ...
            plt.plot(range(len(self.fittedValues)),
                     self.fittedValues, 'r', label="Trained")
            plt.plot(range(len(self.Y)), self.Y, 'g', label="Input")
            plt.grid()
...
def forwardHalfPass(ANFISObj, Xs):
    ...
    for pattern in range(len(Xs[:, 0])):
        ...
        # layer two
        miAlloc = [[layerOne[x][ANFISObj.rules[row][x]] for x in range(
            len(ANFISObj.rules[0]))] for row in range(len(ANFISObj.rules))]
        ...
        for alpha in sorted(ANFISObj.memFuncs[columnX][MF][1].keys()):
            ...
            for rowX in range(len(ANFISObj.X)):
                ...
                for colY in range(ANFISObj.Y.ndim):
                    ...
                    dW_dAplha = senSit * np.array([np.prod([ANFISObj.memClass.evaluateMF(
                        tmpRow)[c][ANFISObj.rules[r][c]] for c in adjCols]) for r in rulesWithAlpha])

                    bucket1 = np.empty(len(ANFISObj.rules[:, 0]))
                    for consequent in range(len(ANFISObj.rules[:, 0])):
                        fConsequent = np.dot(np.append(ANFISObj.X[rowX, :], 1.), ANFISObj.consequents[(
                            (ANFISObj.X.shape[1] + 1) * consequent):(((ANFISObj.X.shape[1] + 1) * consequent) + (ANFISObj.X.shape[1] + 1)), colY])
                        acum = 0
                        if consequent in rulesWithAlpha:
                            acum = dW_dAplha[np.where(
                                rulesWithAlpha == consequent)] * theWSum[rowX]

                        acum = acum - theW[consequent,
                                           rowX] * np.sum(dW_dAplha)
                    ...
                    if ANFISObj.Y.ndim == 1:
                        bucket2[colY] = sum1 * \
                            (ANFISObj.Y[rowX]-theLayerFive[rowX, colY])*(-2)
                    else:
                        bucket2[colY] = sum1 * \
                            (ANFISObj.Y[rowX, colY] -
